...
"""This script helps to normalize/sanitize the number of columns in a CSV file.
Use it on the YEAR.csv files extracted from the DUKweb dataset files
(YEAR.csv.zip).

Why this is necessary: The DUKweb csvs appear to be malformed. Specifically,
the number of columns varies from row to row. This seems to be because the
authors forgot to quote cells that contain commas (this is specifically a
problem for the first column, the vocab keys/tokens, which sometimes contain
commas). This script will merge the left-hand side into one column, so that all
...
def normalize_file(infilepath, columns:int=EXPECTED_NUM_OF_COLUMNS, outfilepath=None):
    if outfilepath == None:
